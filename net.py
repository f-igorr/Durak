...
def think (input_array: NDArray, brain: List[List[NDArray]], activate: Callable= np.tanh) -> int:
    ...
    # теперь оставляем вероятности только для карт кот есть у меня, 
    # умножая рез на бинарный массив моих карт (0 - нет карты, 1 - есть карта)
    # my_card = np.array(convert_listcard_to_binlist(hand))
    res = res * input_array[SLICE_MY_CARDS] # shape(36,1) = shape(36,1)*shape(36,1)  #dont use np.dot
    return res.argmax()